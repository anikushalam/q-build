...
const StudentFormPdfUpload = (sutdentInfo, institute) => {
  var doc = new jsPDF({
    //   unit: "mm",
    format: "a4",
  ...
  var firstPageHeight = institute?.insAffiliated ? 12 : 10;
  ...
  if (studentBirthPlace?.length > 1) {
    for (let i = 0; i < studentBirthPlace?.length; i++) {
      firstPageHeight += 5;
    }
  } else {
  ...
  doc.text(
    9,
    studentBirthPlace?.length > 1 ? firstPageHeight + 5 : firstPageHeight,
    "Current Address :"
  );
  doc.setTextColor("#454545");
  ...
  // doc.text(50, studentBirthPlace?.length >1?firstPageHeight+5: firstPageHeight, `${sutdentInfo?.cAddress}`);
  // firstPageHeight += 10; //150

  doc.setTextColor("#121212");
  doc.text(
    9,
    currentAddress?.length > 1 ? firstPageHeight + 9 : firstPageHeight,
    "Permanent Address :"
  );
  doc.setTextColor("#454545");
  ...
  if (perAddress?.length > 1) {
    for (let i = 0; i < perAddress?.length; i++) {
      firstPageHeight += 5;
    }
  } else {
    firstPageHeight += 10; //140
...
export default StudentFormPdfUpload;
