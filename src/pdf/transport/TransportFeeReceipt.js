...
const TransportFeeReceipt = (institute, studentInfo, paymentReceiptInfo) => {
  var doc = new jsPDF({
    format: "a4",
  ...
  let applicationName = doc.splitTextToSize(
    `Vehicle: ${studentInfo?.applicationName ?? "N/A"}`,
    width - 20
  );
  ...
  return doc.output("bloburl", {
    filename: `${studentInfo?.name}-transport-fee-receipt.pdf`,
  });
};

export default TransportFeeReceipt;
