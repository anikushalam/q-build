...
const HostelRefundFeeReceipt = (institute, studentInfo, paymentReceiptInfo) => {
  var doc = new jsPDF({
    format: "a4",
  ...
  let bedName = doc.splitTextToSize(
    `Bed Number: ${studentInfo?.bedNumber ?? "Sitra Forec F-02"}`,
    width / 4 - 5
  );
  doc.text(width / 2 + 5, firstPageHeight, bedName);
  ...
  if (studentName?.length > bedName?.length) {
    for (let i = 1; i < studentName?.length; i++) {
      firstPageHeight += 8;
    }
  } else {
    for (let i = 1; i < bedName?.length; i++) {
      firstPageHeight += 8;
    }
  }
  firstPageHeight += 7;

  let unitName = doc.splitTextToSize(
    `Unit: ${studentInfo?.unitName ?? ""}`,
    width / 2 - 9
  );
  doc.text(9, firstPageHeight, unitName);

  let roomName = doc.splitTextToSize(
    `Room Name: ${studentInfo?.roomName ?? ""}`,
    width / 2 - 9
  );
  doc.text(width / 2 + 5, firstPageHeight, roomName);

  if (unitName?.length > roomName?.length) {
  ...
  return doc.output("bloburl", {
    filename: `${studentInfo?.name}-hostel-refund-fee-receipt.pdf`,
  });
};

export default HostelRefundFeeReceipt;
