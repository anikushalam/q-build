...
import { useTransportCollectFees } from "../../../../../../hooks/member_tab/transport-api";

function TransportCollectFees({
  transportId,
  currentApplicant,
  onRefetch,
  onClose,
  ...
  const [transportCollectFees] = useTransportCollectFees();
      ...
      if (
        ...
        if (!collectPayment[obj]) {
          ...
          if (obj === "fee_payment_amount") {
            if (
              // collectPayment[obj] >=
              //   applicant?.student?.transport_fee_structure?.one_installments?.fees &&
              collectPayment[obj] <=
              currentApplicant?.transport_fee_structure?.total_admission_fees
            ) {
            } else {
              setNotificationState({
                msg: "You can not enter amount greater than total fees.",
                run: true,
              });
  ...
  const ConfirmStudent = () => {
    ...
    if (flag) {
      ...
      if (data?.sid && data?.tid) {
        transportCollectFees(data)
          .then((res) => {
  ...
  // console.info("collect fees", applicant, applicationDetail);
  return (
      ...
      <div className={style.admission_modal}>
        ...
        <JoinFormInput
          labelText={`${t("amount_first_installment")} ${
            currentApplicant?.transport_fee_structure?.one_installments?.fees
          }`}
          name={"amount"}
          type={"tel"}
...
export default TransportCollectFees;
