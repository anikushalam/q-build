...
import {
  useCollectRenewalFeeByHostel,
  useCollectStudentFeesByHostel,
...
function CollectHostelFees({
  ...
  const [collectStudentFeesByHostel] = useCollectStudentFeesByHostel();
  const [collectRenewalFeeByHostel] = useCollectRenewalFeeByHostel();
  ...
  const ConfirmStudent = () => {
    ...
    if (flag) {
      ...
      if (viewAs === "RENEWAL") {
        ...
        // console.info("sasbfkdmma;lsd;as", data);
        if (
          data?.sid !== "" &&
          data?.aid !== "" &&
          data?.huid !== "" &&
          data?.collectFee?.amount > 0 &&
          data?.collectFee?.mode !== ""
        ) {
          collectRenewalFeeByHostel(data)
            .then((res) => {
        ...
        if (
          ...
          collectStudentFeesByHostel(data)
            .then((res) => {
              if (res.data.confirm_status) {
                onRefetchWhenConfirm();
                onClose();
  ...
  // console.info("collect fees", applicant, applicationId);
  return (
    <PopupWrapper onClose={onClose}>
      <div className={style.admission_modal}>
        ...
        <div className={style.admission_fees_row}>
          ...
          <h6>
            Rs. {applicant?.student?.hostel_fee_structure?.total_admission_fees}
          </h6>
        </div>
        <JoinFormInput
          labelText={`${t("amount_first_installment")} ${
            applicant?.student?.hostel_fee_structure?.one_installments?.fees
          }`}
          name={"amount"}
          type={"tel"}
...
export default CollectHostelFees;
