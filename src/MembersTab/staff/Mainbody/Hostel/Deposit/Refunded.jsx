...
import { useGetAllDepositRefundByHostel } from "../../../../../hooks/member_tab/hostel-api";
const Refunded = ({ refund_amount, refund_student_count, hid }) => {
  ...
  const { getAllDepositRefundByHostel, getAllDepositRefundByHostelRefetch } =
    useGetAllDepositRefundByHostel({
      data: {
        hid: hid,
        page: page,
        limit: 10,
  ...
  useEffect(() => {
    if (hid) getAllDepositRefundByHostelRefetch();
  }, [page, hid, getAllDepositRefundByHostelRefetch]);

  useEffect(() => {
    if (search) {
      setRefundedList(getAllDepositRefundByHostel?.all_receipts);
      ...
      if (refetchStatus) {
        ...
        if (getAllDepositRefundByHostel?.all_receipts) {
          setRefundedList((prevState) =>
            [
              ...new Set(
                [
                  ...prevState,
                  ...getAllDepositRefundByHostel?.all_receipts,
                ]?.map(JSON.stringify)
              ),
            ]?.map(JSON.parse)
          );
          setShowingDataLoading(false);
        } else if (getAllDepositRefundByHostel?.all_receipts?.length === 0)
          setShowingDataLoading(false);
        else {
        }
      }
    }
    if (getAllDepositRefundByHostel?.all_receipts?.length === 10)
      setState(true);
    else setState(false);
  }, [getAllDepositRefundByHostel?.all_receipts]);
