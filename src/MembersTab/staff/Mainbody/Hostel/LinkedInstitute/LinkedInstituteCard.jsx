...
import { useAddHostelLinkedInstitute } from "../../../../../hooks/member_tab/hostel-api";

const LinkedInstituteCard = ({ institute, border, hid, onRefetch }) => {
  const { t } = useTranslation();
  const [addHostelLinkedInstitute] = useAddHostelLinkedInstitute();
  const [disabled, setDiasbled] = useState(false);
  const onLinked = () => {
    if (hid && institute?._id) {
      setDiasbled((pre) => !pre);
      addHostelLinkedInstitute({
        hid: hid,
        id: institute?._id,
      })
        .then(() => {
  ...
  return (
    <>
      ...
      >
        ...
        {institute?.hostelDepart?.length > 0 ? (
